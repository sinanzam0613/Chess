...
namespace MyGame{
    ...
    bool ShowCharacterStatus::init(){
        ...
        //  タッチイベントの登録
        bool* t_this_isActive = &m_isActive;
        std::function<bool(Touch*, Event*)> t_beganFunc = [t_this_isActive](cocos2d::Touch* touch, cocos2d::Event* event){
            ...
            if((*t_this_isActive) && t_rect.containsPoint(t_touchLocation)){
                LOG_CUSTOM("States Touch : %f", t_touchLocation.x);
            }
            return true;
        };
        
        Assistant::AddEventListenerTouchOneByOne(t_beganFunc, NULL, NULL, NULL, m_showSprite);
